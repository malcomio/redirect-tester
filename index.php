...
<div class="container">
  ...
  <p>Upload a CSV file containing original and expected redirected URLs to test
    whether your redirect rules are working as intended.</p>
  <p>See <a
      href="https://github.com/malcomio/redirect-tester/blob/master/README.md">the
  ...
  if (!array_key_exists('csv_input', $_FILES)) {
    ...
    $duplicate_originals = array_filter(array_count_values($originals),
      'more_than_1');

    $invalid_originals = array_filter(array_column($results, 'original'),
      'invalid_url');
    $invalid_expecteds = array_filter(array_column($results, 'expected'),
      'invalid_url');
    ?>
      ...
      <span class="glyphicon glyphicon-exclamation-sign"
        ...
        <ul>
          <?php foreach ($invalid_originals as $url): ?>
            ...
            foreach ($instances as $instance): ?>
              <li>
                Row <?php print $results[$instance]['row']; ?>
                : <?php print $url; ?>
              </li>
            <?php endforeach; ?>
          <?php endforeach; ?>
